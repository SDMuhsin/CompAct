...
#
# Gap-fill (submit ONLY the arms missing from results/mo53_glue.csv):
#   RERUN_ONLY=1 RERUN_TIERS="1"   ./sbatch/run_tinyllama.sh   # cb+rte+mrpc  (36 arms)
#   RERUN_ONLY=1 RERUN_TIERS="2"   ./sbatch/run_tinyllama.sh   # stsb+boolq   (20 arms)
#   RERUN_ONLY=1 RERUN_TIERS="3"   ./sbatch/run_tinyllama.sh   # cola v3      (12 arms)
#   RERUN_ONLY=1 RERUN_TIERS="4"   ./sbatch/run_tinyllama.sh   # sst2 v3+qnli (14 arms, split)
# Recover orphaned per-seed partials as an Mo<n> row without retraining:
#   AGG_ONLY=1 RERUN_ONLY=1 RERUN_TIERS="4" ./sbatch/run_tinyllama.sh
#
# ============================================================================
...
# RERUN_ONLY=1 to submit a hand-picked set of arms after a partial sweep, without
# re-spending compute on the arms that already landed in the CSV.
#
# In RERUN_ONLY mode the submitted set is defined ENTIRELY by RERUN_PAIRS: the
# `techniques` and `tasks` loop lists are DERIVED from the pairs further below
# (see "derive loop lists"). A commented-out entry in `tasks`, a V3_ONLY setting
# or a V3_MODES that omits a mode therefore can NOT silently drop a requested
# arm — which is what a plain filter-on-top-of-the-cross-product would do.
#
# --- GAP TIERS (snapshot audited 2026-08-01 against results/mo53_glue.csv) ---
# The full 4-arm matrix is {baseline, _flash (v2), v3 recompute, v3 int4} x
# 6 techniques x 8 tasks = 192 rows; 110 exist, 82 are missing. A task with only
# v3 rows (or only baseline+v2 rows) yields no comparison, so the tiers below are
# ordered to COMPLETE whole tasks cheapest-first. Pick tiers with RERUN_TIERS.
RERUN_ONLY="${RERUN_ONLY-0}"

# Tier 1 — cb/rte/mrpc: baseline + v2 never ran for these (the July sweep was
# V3_ONLY). 36 arms, all non-split, <=9h each. Completes 3 tasks.
GAP_TIER1=(
    "base:cb"     "base_flash:cb"     "lora:cb"     "lora_flash:cb"
    "dora:cb"     "dora_flash:cb"     "adalora:cb"  "adalora_flash:cb"
    "dylora:cb"   "dylora_flash:cb"   "vera:cb"     "vera_flash:cb"
    "base:rte"    "base_flash:rte"    "lora:rte"    "lora_flash:rte"
    "dora:rte"    "dora_flash:rte"    "adalora:rte" "adalora_flash:rte"
    "dylora:rte"  "dylora_flash:rte"  "vera:rte"    "vera_flash:rte"
    "base:mrpc"   "base_flash:mrpc"   "lora:mrpc"   "lora_flash:mrpc"
    "dora:mrpc"   "dora_flash:mrpc"   "adalora:mrpc" "adalora_flash:mrpc"
    "dylora:mrpc" "dylora_flash:mrpc" "vera:mrpc"   "vera_flash:mrpc"
)
# Tier 2 — stsb/boolq: same gap MINUS dylora_flash + vera_flash, which already
# landed in the June sweep. 20 arms, <=21h each. Completes 2 more tasks.
GAP_TIER2=(
    "base:stsb"   "base_flash:stsb"   "lora:stsb"    "lora_flash:stsb"
    "dora:stsb"   "dora_flash:stsb"   "adalora:stsb" "adalora_flash:stsb"
    "dylora:stsb" "vera:stsb"
    "base:boolq"  "base_flash:boolq"  "lora:boolq"   "lora_flash:boolq"
    "dora:boolq"  "dora_flash:boolq"  "adalora:boolq" "adalora_flash:boolq"
    "dylora:boolq" "vera:boolq"
)
# Tier 3 — cola: the mirror-image gap (baseline + v2 exist from March, no v3).
# 12 arms, <=18h each. Completes CoLA.
GAP_TIER3=(
    "base_flashffnv3_recompute:cola"    "base_flashffnv3_int4:cola"
    "lora_flashffnv3_recompute:cola"    "lora_flashffnv3_int4:cola"
    "dora_flashffnv3_recompute:cola"    "dora_flashffnv3_int4:cola"
    "adalora_flashffnv3_recompute:cola" "adalora_flashffnv3_int4:cola"
    "dylora_flashffnv3_recompute:cola"  "dylora_flashffnv3_int4:cola"
    "vera_flashffnv3_recompute:cola"    "vera_flashffnv3_int4:cola"
)
# Tier 4 — the expensive tail: sst2 v3 arms (same mirror-image gap as cola) plus
# the two Full-FT QNLI arms the June sweep never wrote. 14 arms, ALL split-by-seed
# (sst2 + qnli are in SPLIT_BY_SEED_TASKS) -> 28 submissions, days of GPU each.
GAP_TIER4=(
    "base_flashffnv3_recompute:sst2"    "base_flashffnv3_int4:sst2"
    "lora_flashffnv3_recompute:sst2"    "lora_flashffnv3_int4:sst2"
    "dora_flashffnv3_recompute:sst2"    "dora_flashffnv3_int4:sst2"
    "adalora_flashffnv3_recompute:sst2" "adalora_flashffnv3_int4:sst2"
    "dylora_flashffnv3_recompute:sst2"  "dylora_flashffnv3_int4:sst2"
    "vera_flashffnv3_recompute:sst2"    "vera_flashffnv3_int4:sst2"
    "base:qnli"                         "base_flash:qnli"
)

# RERUN_TIERS: space-separated subset of {1 2 3 4}; only consulted when
# RERUN_ONLY=1. Default = all four.
RERUN_TIERS="${RERUN_TIERS-1 2 3 4}"
RERUN_PAIRS=()
for _tier in $RERUN_TIERS; do
    case $_tier in
        1) RERUN_PAIRS+=("${GAP_TIER1[@]}") ;;
        2) RERUN_PAIRS+=("${GAP_TIER2[@]}") ;;
        3) RERUN_PAIRS+=("${GAP_TIER3[@]}") ;;
        4) RERUN_PAIRS+=("${GAP_TIER4[@]}") ;;
        *) echo "ERROR: invalid RERUN_TIERS entry '$_tier' (allowed: 1 2 3 4)"; exit 1 ;;
    esac
done

# --- AGG_ONLY: submit ONLY the CPU aggregation job for the selected arms ------
# Default 0. Set AGG_ONLY=1 to recombine per-seed partials that are already on
# disk WITHOUT retraining and WITHOUT clearing ./results/_partial/<run_name>.
# Use when an array finished (or partly finished) but its aggregation never ran,
# so the seeds are sitting in _partial with no CSV row. Aggregating <5 seeds is
# allowed and writes an honest Mo<n> row (the seed column lists exactly which).
# Only meaningful for tasks in SPLIT_BY_SEED_TASKS; other arms are skipped.
AGG_ONLY="${AGG_ONLY-0}"

# --- CLOBBER_PARTIALS: allow a submit to delete existing per-seed partials ---
# Default 0 = the pre-flight check below ABORTS (before submitting anything) if
# any selected arm still has seed_*.json files on disk, which would otherwise be
# wiped by the `rm -rf $partial_dir` that precedes every array submission.
CLOBBER_PARTIALS="${CLOBBER_PARTIALS-0}"

# --- ALLOW_QUEUED_DUPES: allow resubmitting an arm already in the SLURM queue --
# Default 0 = the pre-flight check ABORTS if any selected arm's job name is
# already queued/running (a duplicate array would race it for the same partial
# dir and CSV row). Only set this if you have just scancel'd the original.
...
# --- RERUN_ONLY: derive loop lists from RERUN_PAIRS --------------------------
# Must run AFTER both `techniques` and `tasks` are fully built, since it
# REPLACES them. Every pair is then reachable regardless of which tasks are
# commented out above or which arms V3_MODES/V3_ONLY happened to generate;
# should_submit_pair() still filters the cross-product down to the exact pairs.
if [[ "$RERUN_ONLY" == "1" ]]; then
    _rr_techs=(); _rr_tasks=()
    for _p in "${RERUN_PAIRS[@]}"; do
        _tech="${_p%%:*}"; _task="${_p##*:}"
        if [[ "$_tech" == "$_p" || -z "$_tech" || -z "$_task" ]]; then
            echo "ERROR: malformed RERUN_PAIRS entry '$_p' (expected <technique>:<task>)"; exit 1
        fi
        [[ " ${_rr_techs[*]} " == *" $_tech "* ]] || _rr_techs+=("$_tech")
        [[ " ${_rr_tasks[*]} " == *" $_task "* ]] || _rr_tasks+=("$_task")
    done
    if [[ ${#_rr_techs[@]} -eq 0 ]]; then
        echo "ERROR: RERUN_ONLY=1 but RERUN_PAIRS is empty (check RERUN_TIERS='$RERUN_TIERS')"; exit 1
    fi
    techniques=("${_rr_techs[@]}")
    tasks=("${_rr_tasks[@]}")
fi
...
echo "Rerun only: ${RERUN_ONLY} ($([[ "$RERUN_ONLY" == "1" ]] && echo "restricted to the ${#RERUN_PAIRS[@]} pairs from RERUN_TIERS='${RERUN_TIERS}'" || echo "full technique x task cross-product"))"
echo "Agg only:   ${AGG_ONLY} ($([[ "$AGG_ONLY" == "1" ]] && echo "aggregation jobs ONLY — no training, partials preserved" || echo "normal: train then aggregate"))"
echo "============================================"
echo ""
...
count_partials() {
    # $1 = partial dir. Echoes the number of seed_*.json files (0 if none).
    local d=$1
    [[ -d "$d" ]] || { echo 0; return; }
    local n
    n=$(find "$d" -maxdepth 1 -name 'seed_*.json' 2>/dev/null | wc -l)
    echo "${n// /}"
}

# --- PRE-FLIGHT: don't clobber partials, don't duplicate in-flight arms -------
# Two independent hazards, both checked BEFORE any job is submitted:
#
# (a) Completed per-seed partials. A normal submit does
#     `rm -rf ./results/_partial/<run_name>` so a resubmit can not mix stale
#     seeds. Correct for a fresh arm, CATASTROPHIC for an arm whose array is
#     still running or whose aggregation is still pending — those seeds are
#     hours-to-days of GPU each and are NOT recoverable. AGG_ONLY never clears,
#     so it is exempt from this one.
# (b) An arm already in the queue. A partial is written only when a seed
#     FINISHES, so an array that is queued or mid-training is invisible to (a).
#     Resubmitting it would run a second array against the same partial dir and
#     the same CSV row. Detected by matching job names in squeue.
if [[ "$LOCAL_MODE" != true ]]; then
    ...
    for technique in "${techniques[@]}"; do
        for task in "${tasks[@]}"; do
            ...
            _job="${MODEL_SHORT}_${technique}_${task}"
            if [[ -n "$queued_names" ]] \
               && grep -qxF -e "$_job" -e "${_job}_seed" -e "${_job}_agg" <<< "$queued_names"; then
                in_queue+=("$_job")
            fi
            [[ "$AGG_ONLY" == "1" ]] && continue
            should_split_by_seed "$task" || continue
            _d="./results/_partial/${technique}_${MODEL_SHORT}_${task}"
            _n=$(count_partials "$_d")
    ...
    if [[ ${#in_queue[@]} -gt 0 ]]; then
        ...
        echo "       row). Wait for them, or narrow the selection. Override: ALLOW_QUEUED_DUPES=1."
    ...
    if [[ ${#at_risk[@]} -gt 0 ]]; then
        ...
        printf '         %s\n' "${at_risk[@]}"
        echo ""
        echo "       If an array/aggregation for these is still in the queue (squeue -u \$USER),"
        echo "       do NOT clobber them. To turn existing partials into an honest Mo<n> CSV row"
        echo "       without retraining:   AGG_ONLY=1 RERUN_ONLY=1 RERUN_TIERS=<tier> $0"
        echo "       To discard them and retrain from scratch: re-run with CLOBBER_PARTIALS=1."
...
for technique in "${techniques[@]}"; do
    ...
    for task in "${tasks[@]}"; do
        ...
        n_partials=$(count_partials "./results/_partial/${run_name}")

        if [[ "$AGG_ONLY" == "1" ]]; then
            if ! should_split_by_seed "$task"; then
                echo "  [SKIP] ${job_name} — AGG_ONLY=1 but '$task' is not split by seed (no partials are ever written)"
                continue
            fi
            if [[ "$n_partials" -eq 0 ]]; then
                echo "  [SKIP] ${job_name} — AGG_ONLY=1 but ./results/_partial/${run_name} holds no seed_*.json"
                continue
            fi
        fi
        ...
        if should_split_by_seed "$task"; then
            ...
            if [[ "$AGG_ONLY" == "1" ]]; then
                # Aggregate what is already on disk; train nothing, delete nothing.
                echo "  [AGG_ONLY] $job_name  ($n_partials partial seed(s) in $partial_dir)"
                array_id=""
            else
...
EOF
            ...
            # Aggregation waits on the array when there is one; under AGG_ONLY it
            # runs immediately against the partials already on disk.
            agg_dep=""
            agg_src="depends on array $array_id"
            if [[ -z "$array_id" ]]; then
                agg_src="AGG_ONLY: $n_partials partial(s) already on disk"
            else
                agg_dep="--dependency=afterany:${array_id}"
            fi

            agg_id=$(sbatch --parsable $agg_dep <<EOF
#!/bin/bash
...
echo '========================================'
echo "Aggregating Mo5: $run_name  ($agg_src)"
echo "Started: \$(date)"
echo '========================================'
...
EOF
            ...
            if [[ -z "$array_id" ]]; then
                echo "  [agg ${agg_id}] $job_name  ($technique_desc, ${task}, AGG-ONLY over $n_partials partial seed(s))"
