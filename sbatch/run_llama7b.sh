...
# WARNING (measured 2026-08-02, boring_baseline_frontier.md §4.1): with gradient checkpointing
# ON, FlashFFN v3 saves EXACTLY 0.00 MiB -- gc alone and v3_recompute+gc peak at 3272.93 MiB
# both (non-reentrant) and 3273.18 MiB both (reentrant), bit-identical -- while costing
# +8.32..+8.65% step time. CONTEXT.md §27 previously said "savings vanish (-1.0%)"; the real
# figure is 0.0%. On THIS script the v3 arms are therefore pure overhead. Left in place
# deliberately: this script has never run (0 LLaMA-7B rows), and dropping gc would change the
# memory envelope the 7B config was sized for. If these arms are ever submitted, either drop
# --gradient_checkpointing or expect v3 to cost ~8.5% throughput for nothing.
